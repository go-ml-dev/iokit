...
func (uw *urlwriter) Commit() (err error) {
	...
	if err = uw.Whole.Commit(); err != nil { return }
	if uw.iourl.Cache.Defined() {
		// file was closed in Commit call
		if err = os.Rename(uw.iourl.Cache.Path()+"~",uw.iourl.Cache.Path()); err != nil { return }
		if rd, err = File(uw.iourl.Cache.Path()).Open(); err != nil { return }
	} else {
